...
class _AnalysisDetailsScreenState extends State<AnalysisDetailsScreen> {
  ...
  void didChangeDependencies() {
    ...
    final analysis = ModalRoute.of(context)?.settings.arguments as Map<String, dynamic>? ?? {};
    final String type = analysis['type'] ?? 'taule';
    final bool isVideo = type == 'video';

    if (isVideo && _videoController == null) {
      final int id = analysis['id'];
      final videoUrl = "http://192.168.1.8:8000/garage/taule/video/$id";
      _initVideo(videoUrl);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final String? analysedPath = analysis['analysed_path'];
    final String? imagePath = analysis['analysed_image'];

    final String imageUrl = (!isVideo && (analysedPath ?? imagePath) != null)
        ? "http://$baseurl:8000/images/${analysedPath ?? imagePath}"
        : "";

    final raw = analysis['detections'];
